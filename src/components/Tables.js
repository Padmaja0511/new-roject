...
import { faAngleDown, faAngleUp, faArrowDown, faArrowUp, faEdit, faEllipsisH,  faExternalLinkAlt, faEye,faTimes, faTrashAlt, faCheck } from '@fortawesome/free-solid-svg-icons';
...
export const PageVisitsTable = () => {
  const TableRow = (props) => {
    ...
    return (
      <tr>
        <th scope="row">{props.hostname}</th>
        <td>{props.ip_address}</td>
        <td>{props.device_family}</td>
        <td>{props.region}</td>
        {props.status?.split(" ")[0] == 'Reachable' ? (
          <td><FontAwesomeIcon icon={faCheck} className={`text-success me-3`} />
          Reachable</td>
        ): (<td><FontAwesomeIcon icon={faTimes} className={`text-danger me-3`} />
        Unreachable</td>
        )}
        <td>{props?.status.split(' ')[2]?.slice(0, -1)}<FontAwesomeIcon icon="fa fa-refresh" className={` me-3`} /></td>
      </tr>
    );
  };
      ...
      <Card.Header>
        <Row className="align-items-center">
          ...
          <Col className="text-end">
            <Button variant="secondary" size="sm">Refresh all</Button>
          </Col>
        </Row>
      </Card.Header>
      <Table responsive className="align-items-center table-flush">
        <thead className="thead-light">
          <tr>
            ...
            <th scope="col">Last Checked Status</th>
          </tr>
        </thead>
        <tbody>
          {devices.map(pv => <TableRow key={`page-visit-${pv.id}`} {...pv} />)}
        </tbody>
      </Table>
    </Card>
  );
